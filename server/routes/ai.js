...
async function chat(base, type, model, messages, maxTokens = 512) {
...
export function createAIRoutes(store) {
  const router = Router();
  ...
  router.post('/summarize', async (req, res) => {
    ...
    const recentActivity = activity.slice(0, 15).map(a => `- ${a.message}`);
    const recentSessions = sessions.slice(0, 3).map(s => `- [${s.project}] ${s.summary.slice(0, 80)}`);
    const openTaskList = tasks.filter(t => t.status !== 'done').map(t => t.title).join(', ') || 'none';
