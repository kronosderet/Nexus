...
fuel usage, and git state across all projects. Features:
  ...
  • 26 native MCP tools — every Nexus operation callable from Claude Desktop natively
...
`);
  ...
  ${dim('Exposes the Nexus metabrain as 26 native MCP tools so every Claude')}
...
`);
  ...
  ${amber('Commands:')}
    ...
    nexus sync                     Fetch all remotes + show fleet status
    nexus commit-all "message"     Commit all dirty repos at once
